...
Gatsby를 설치한 뒤 여러가지 스타터 중 하나를 선택해서 시작할 예정인데, minimal-blog를 사용하여 개발하려고 한다.
...
여기까지 `minimal-blog`라는 이름의 폴더가 만들어지고, gatsby develop을 입력하면 로컬호스트에 dev환경이 만들어진다.

참조한 블로그에서는 Gatsby에서 사용하는 쉐도윙이라는 기법으로 커스터마이징을 바로 시작했지만,
처음 사용해보는 나에게는 큰 장벽이라.. 나중에 적용해보도록 하고 이어서 배포하는 방법을 소개하려고한다.
...
전자의 경우에는 많은 자료가 있으니 찾아서 해보면 좋을 것 같고, 더 간편하다. 대신 홈페이지가 netlify.app으로 나온다는점?
나 같은 경우는 새로운걸 너무많이써서 그냥 후자로 배포하기로 했다.
...
이를 정적페이지로 빌드해서 배포하면 index.html로 화면을 보여주게 된다.
...
일단 gatsby 프로젝트를 개발용 브랜치로 바꾸도록하자.
...
여기까지 실행을 했다면, minimal-blog가 당신이 설정한 github pages로 배포가 되었을 것이다.

이제 gatsby develop 브랜치로 돌아와서 하나씩 수정하면서 커스터마이징하면 된다.
아래 블로그 또한 gatsby를 통해서 만들어진 블로그이며, 커스터마이징 방법에 대해서 소개하고 있으니 참고하면 좋을것 같다.
...
내가 배포한 방식을 간략하게 (생략한 부분이 정말 많다..) 적어봤는데 중간과정을 다 날려버린것 같아서 뭔가... 글이 아쉽다.  
